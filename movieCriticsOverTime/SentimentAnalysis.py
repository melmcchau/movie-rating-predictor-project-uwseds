...
#         content=quote.groupby('critic')['quote'].apply(' '.join).reset_index()['quote']
    ...
    ### calculate total number or words
        ...
        Words_List_for_Each_Emotion=SentimentSetup.words_list_for_each_emotion(length_filtered)
    
        result_list=[]
        for i in EMOTIONS:
            result={}
            example_words=[SentimentSetup.get_common_words_list(value)[:3] for key,value in Words_List_for_Each_Emotion.items() if key==i]
            result['EMOTION']=i
